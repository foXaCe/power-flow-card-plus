...
function _getCircleEdgePointInternal(centerX: number, centerY: number, radius: number, targetX: number, targetY: number): { x: number; y: number } {
...
function createLine(
  ...
  const dot =
    showDots && checkShouldShowDots(config) && power > 1
      ? (() => {
...
export const flowElement = (
  main: PowerFlowCardPlus,
  config: PowerFlowCardPlusConfig,
  { battery, grid, solar, newDur, dailyExport, dailyCost }: Flows
) => {
